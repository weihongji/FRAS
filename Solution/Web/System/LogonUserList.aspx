<%@ Page Title="登录帐号管理" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="LogonUserList.aspx.cs" Inherits="System_LogonUserList" %>
...
<script type="text/javascript">
	$(function () {
		...
		$("#btnAdd").click(function () {
			location.href = "LogonUserForm.aspx?id=0";
		});

		$("#btnDelete").click(function () {
			var selectedCount = $(".list :checkbox:checked").length;
			var msg = "你确定要删除选中的登录帐户吗？";
			if (selectedCount > 1) {
				msg = "你确定要删除选中的（" + $(".list :checkbox:checked").length + "个）登录帐户吗？"
			}
			if (!confirm(msg)) {
				return;
			}
			$(".list :checkbox:checked").each(function () {
				var id = this.value;
				$.post("LogonUserAjax.aspx?type=delete", { id: id }, function (response) {
					if (response != "true") {
						alert("删除失败！没有找到编号为" + id + "的登录帐户。");
					}
				});
			});
	...
	<table class="list">
		...
		<tbody>
			...
			<tr>
				...
				<td><%#Eval("ID") %></td>
				<td><%#Eval("logonUser")%></td>
				<td><%#Eval("RoleName")%></td>
				<td><%#Eval("DeptName")%></td>
				<td class="c"><%#Eval("ActiveName")%></td>
				<td class="c"><a href="LogonUserForm.aspx?id=<%#Eval("ID") %>">修改</a></td>
			</tr>
			</ItemTemplate>
			</asp:Repeater>
		</tbody>
